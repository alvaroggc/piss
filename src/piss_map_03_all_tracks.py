...
# -*- coding: utf-8 -*-
#
# ~/projects/piss/src/piss_map_03_all_tracks.py
#
# info  : show all tracks of cyclonic activity over southern hemisphere
#         in piss simulations (cesm2), based on Hanley & Caballero (2012).
# usage : ./piss_map_03_all_tracks.py [simid]
# author: @alvaroggc
...
dirimg  = f'{homedir}/projects/piss/img/tracks_all/{nocoast[1:]}'
...
output = f'piss_map{pis_sa}_tracks_{simid.lower()}{nocoast}.png'
...
for tid in tracks.keys():
    ...
    xp = np.array([p[0] for p in tracks[tid]])       # x-coordinate
    yp = np.array([p[1] for p in tracks[tid]])       # y-coordinate
    tp = cf.dt_vector([p[5] for p in tracks[tid]], calendar=calendar)  # t-coordinate
    ...
    # # skip track if it doesn't go over
    # if pis:
    #
    ...
    # logging message
    print(f'{indent}drawing {tid}')

    # plot track
    ax.plot(xp, # xp_interp,
            yp, # yp_interp,
            transform=transxy,
            zorder=2,
            ls='-', alpha=0.05, color='darkred')
